...
# 大屏打卡程序（客户端）
# 版权所有 (c) 2025 - Now 刘宇晨
# Open code License : GNU GPL v3
# 我（刘宇晨）保留对该版本代码的所有权利。不对第三方的任何修改版本做任何保证。
...
class AttendanceApp:
    def __init__(self, window):
        ...
        self.offline_mode = False   # 单机离线模式标志

        # 未配置服务器地址时直接进入单机离线模式
        if not self.online_ip:
            self.offline_mode = True
            self.server_status = "单机离线"
        ...
        # 先让窗口完成渲染，再设置窗口样式
        # 这对打包为 windowed 模式（console=False）至关重要：
        # 只有 update() 之后 winfo_id() / FindWindowW 才能找到真实的顶层 hwnd
        self.window.update()
    ...
    def save_all_settings(self, settings_window):
        try:
            ...
            messagebox.showinfo("提示", "所有设置已保存并生效，请重启程序以生效")
            exit(0)
    ...
    def check_server_status(self):
        if not self.online_mode or not self.online_ip:
            ...
            if hasattr(self, 'online_status_label'):
                self.online_status_label.config(
                    text=f"服务器状态: {self.server_status} (本地数据已保存)",
                    fg=self.offline_color)
            return
        ...
        if response is None:
            ...
            for m in machines:
                if m['uuid'] == self.client_uuid:
                    ...
                    self.offline_mode = (self.server_status != "在线")
                    found = True
                    break
            if not found:
        ...
        if hasattr(self, 'online_status_label'):
            if self.server_status == "在线":
                label_text = f"服务器状态: {self.server_status}"
                color = self.online_color
            else:
                label_text = f"服务器状态: {self.server_status} (单机离线模式)"
                color = self.offline_color
            self.online_status_label.config(text=label_text, fg=color)
    ...
    def sync_data_to_server(self):
        if not self.online_mode or self.server_status != "在线":
            # 离线模式下数据已由 save_student_data() 保存到本地，静默跳过
            return
    ...
    def save_remote_settings(self, settings_window):
        """保存远程设置"""
        self.online_ip = self.ip_entry.get().strip()
        if self.online_ip:
            # IP 不为空时，端口和密码为必填，校验端口格式
            try:
    ...
    def mark_attendance(self, name):
        ...
        if self.online_mode and self.server_status == "在线":
            ...
            self.status_var.set(f"{name} 打卡成功! 打卡时间: {current_time}  [单机离线，已保存本地]")
