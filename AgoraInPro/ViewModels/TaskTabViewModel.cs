...
public class TaskTabViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    /// <summary>当前标签是否被激活（L7：标签栏高亮由绑定驱动，不再依赖视觉树遍历）</summary>
    ...
    /// 服务器地址或密码变化时自动重建连接（H1：避免旧连接残留、新配置不生效）
    /// </summary>
    public void UpdateGlobalServerConfig(string ip, int port, string password, string version)
    {
        var configChanged = _globalServerIp != ip || _globalServerPort != port || _globalServerPassword != password;
        _globalServerIp = ip;
        _globalServerPort = port;
        _globalServerPassword = password;

        // 服务器配置变化且开启在线模式时，释放旧连接并重建
        if (configChanged && Config.OnlineMode && !string.IsNullOrEmpty(ip))
        {
            _syncCts?.Cancel();
            _syncCts?.Dispose();
            _syncCts = null;
    ...
    /// <summary>
    /// 停止后台同步并释放资源（H1：显式释放服务器连接，避免后台线程/连接泄漏）
    /// </summary>
    public void Dispose()
    {
        _syncCts?.Cancel();
    ...
    private async Task ApplyServerDataAsync()
    {
        ...
        if (serverData != null && serverData.Count > 0)
            await RunOnUiThreadAsync(() => ApplyServerDataCore(serverData));
    }

    /// <summary>
    /// 将服务器数据应用到本地学生集合（M1：必须由调用方调度到 UI 线程执行）
    /// </summary>
    private bool ApplyServerDataCore(Dictionary<string, StudentAttendance> serverData)
    {
        bool changed = false;
        foreach (var (name, sa) in serverData)
        {
            var stu = Students.FirstOrDefault(s => s.Name == name);
            var firstTime = ParseTime(sa.FirstTime);
            if (stu != null)
            {
                if (stu.FirstTime != firstTime || stu.Count != sa.Count) changed = true;
                stu.FirstTime = firstTime;
                stu.Count = sa.Count;
            ...
            else
            {
                ...
                Students.Add(new StudentModel { Name = name, FirstTime = firstTime, Count = sa.Count, History = sa.History });
                changed = true;
            }
        }
        if (changed)
    ...
    /// H1：离线时主动尝试重连，避免连接失效后每 5 秒空转；
    /// M1：集合与绑定属性更新统一调度到 UI 线程，避免并发集合异常。
    /// </summary>
    ...
    /// <summary>
    /// 将操作调度到 UI 线程执行（M1：ObservableCollection 与绑定属性的更新必须发生在 UI 线程）
    /// </summary>
    ...
    private void ExecuteCheckIn(object? param)
    {
        ...
        stu.FirstTime = now; stu.Count++; stu.History.Add(nowStr);
        PunchedCount = Students.Count(s => s.IsCheckedIn);
        UpdateRanking(); SaveAttendanceData();
        StatusMessage = $"{stu.Name} 打卡成功! 时间: {nowStr}";
    ...
    public void CancelCheckIn(object? param)
    {
        ...
        if (stu.History.Count > 0)
        {
            ...
            if (stu.FirstTime?.ToString("yyyy-MM-dd HH:mm:ss") == removed)
                stu.FirstTime = stu.History.Count > 0 && DateTime.TryParse(stu.History.First(), out var first)
                    ? first : null;
    ...
    public void UpdateRanking()
    {
        ...
        {
            var timeStr = s.FirstTime?.ToString("HH:mm") ?? "--:--";
    ...
    private void LoadAttendanceData()
    {
        ...
        foreach (var line in File.ReadAllLines(_dataFile))
        {
            ...
            if (stu != null)
            {
                ...
                stu.FirstTime = DateTime.TryParse(parts[2], out var ft) ? ft : null;
    ...
    private void ExportData()
    {
        ...
        foreach (var s in Students)
            lines.Add($"{EscapeCsvField(s.Name)},{EscapeCsvField(s.FirstTime?.ToString("yyyy-MM-dd HH:mm:ss") ?? "未打卡")},{s.Count},{EscapeCsvField(string.Join("|", s.History))}");
    ...
    private void ImportData()
    {
        ...
        {
            // H5：使用 CSV 解析器而非 line.Split(',')，支持带逗号的姓名（如 "王,小明"）
            ...
            {
                Name = parts[0],
                FirstTime = parts[1] != "未打卡" && DateTime.TryParse(parts[1], out var ft) ? ft : null,
                Count = int.TryParse(parts[2], out var c) ? c : 0,
                History = parts.Count > 3 ? parts[3].Split('|', StringSplitOptions.RemoveEmptyEntries).ToList() : new()
            };
        }
        ...
        StatusMessage = skipped > 0
            ? $"数据已从 {dlg.FileName} 导入（跳过 {skipped} 行无效数据）"
            : $"数据已从 {dlg.FileName} 导入";
    }

    /// <summary>
    /// 将字符串解析为可空的首次打卡时间（H4）
    /// </summary>
    private static DateTime? ParseTime(string? s) => DateTime.TryParse(s, out var t) ? t : null;

    /// <summary>
    /// 简单的 CSV 行解析器：支持用双引号包裹含逗号的字段（H5），
    /// 例如 "王,小明",2026-08-23 08:00:00,1
    /// </summary>
    ...
    /// <summary>
    /// 将字段转义为合法 CSV 字段：含逗号、引号或换行时用双引号包裹（H5）
    /// </summary>
    ...
    private Dictionary<string, StudentAttendance> GetAttendanceDict() =>
        ...
        { Name = s.Name, Count = s.Count, FirstTime = s.FirstTime?.ToString("yyyy-MM-dd HH:mm:ss"), History = s.History });
