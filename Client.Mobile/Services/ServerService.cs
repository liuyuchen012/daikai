...
/// <summary>
/// Remote server communication service for AgoraIn mobile client.
/// Handles client registration, RSA signature auth, data sync, and config sync.
/// Adapted from WPF client — uses FileSystem.AppDataDirectory for key storage.
/// </summary>
public class ServerService
{
    ...
    private void LoadOrCreateKeys()
    {
        ...
        var keyFile = Path.Combine(appData, "client_key.pem");
        var uuidFile = Path.Combine(appData, "client_uuid.txt");
    ...
    public async Task<bool> RegisterAsync(string machineName)
    {
        try
        {
            ...
            if (serverUuid != _clientUuid)
            {
                ...
                File.WriteAllText(Path.Combine(FileSystem.AppDataDirectory, "client_uuid.txt"), serverUuid);
            }
            return true;
        }
        catch { return false; }
    ...
    public async Task<Dictionary<string, StudentAttendance>?> LoadDataAsync()
    {
        try
        {
            ...
            var body = new { uuid = _clientUuid, task_id = _taskId, signature = Sign(challenge), challenge, password = _password };
