...
[![文档](https://img.shields.io/badge/文档-doc.615mc.top-blue)](https://doc.615mc.top)
...
>
> 首页按设备 UUID 分组展示，每行显示文件夹图标 + 设备名称、在线/离线徽章、任务数、最后在线时间、操作按钮
...
> 详细文档：[doc.615mc.top](https://doc.615mc.top) — 包含完整的安装配置、使用教程、API 说明和常见问题解答。
...
**创建签到流程：**
1. 菜单 → **远程** → **创建签到**
2. 填写签到密码（需重复确认）、教室名称、科目
3. 导入 CSV 学生名单
4. 点击创建，系统生成 6 位短链码和二维码
...
1. 菜单 → **文件** → **清空记录**
2. 输入管理员密码验证身份
3. 第一次确认："确定要清空所有打卡记录吗？"
4. 第二次确认："这是最终确认！此操作不可撤销！"
5. 清空完成

> 通过三层安全机制（密码 + 两次确认）防止误操作。
...
- 任务卡片网格：自动列数布局，每张卡片包含任务名称、统计（总人数 / 已打卡数）、最后更新时间
- 点击任务卡片进入任务详情页
...
- 文件夹节点使用 Segoe MDL2 Assets 字体 `&#xE8B7;`（文件夹图标）
- 文件节点使用 `&#xE8A5;`（文档图标，`#4285f4` 颜色）
...
方法一：使用导出 CSV → 拷贝到新电脑 → 导入 CSV
方法二：直接拷贝 `data/tabs/` 目录到新电脑
方法三：通过服务器同步，在新客户端点"从服务器加载"
</details>
...
本项目基于 **GNU General Public License v3** 开源，详见 [LICENSE](LICENSE) 文件。

```
Copyright (C) 2026 刘宇晨
