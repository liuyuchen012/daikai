...
- 通过「**设置 → 学生列表**」打开当前任务的学生管理对话框（也可用于后台任务名单维护）
- **添加**：输入姓名即可加入当前任务，自动去重；**删除**：删除学生并一并清除其打卡记录
- **编辑**：可修改学生姓名，并勾选该学生**需要进行打卡的任务**（多任务间名单自动同步，改名后旧姓名会从其他任务名单移除）
- **批量删除**：按住 Ctrl / Shift 多选多名学生后一键删除，确认框列出所有将被删除的姓名
- 学生名单保存于任务目录 `name.txt`，编辑后实时写盘并刷新打卡面板与排名
...
- **设置**：学生列表（当前任务，添加/编辑/删除/批量删除）、任务设置（当前标签页）、管理员设置（全局）
- **帮助**：Github、检查版本列表、关于
...
> **管理学生名单**：**设置 → 学生列表** 可添加 / 删除 / 编辑学生（编辑时可修改姓名并勾选需要打卡的任务）；按住 Ctrl 或 Shift 多选后可**批量删除**。
...
- 大屏模式学生列表管理：添加 / 编辑（改姓名 + 勾选需要打卡的任务）/ 删除 / **批量删除**（Ctrl/Shift 多选）
